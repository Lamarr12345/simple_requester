...
class PostWidget(QWidget):
    def __init__(self, parent):
        ...
        label_data = QLabel("Data (dictionary format):")
        self.le_data = QLineEdit(self)

        label_json = QLabel("JSON (JSON format):")
        self.le_json = QLineEdit(self)
        ...
        v_data_layout.addWidget(self.le_data)

        v_json_layout = QVBoxLayout()
        v_json_layout.addWidget(label_json)
        v_json_layout.addWidget(self.le_json)
        ...
        v_base_layout.addLayout(v_data_layout)
    ...
    def save_request(self):
        ...
        data = self.recent_valid_request_data.get("data")
        if data:
            new_key += f"++d:{data}"
        json = self.recent_valid_request_data.get("json")
        if json:
            new_key += f"++j:{json}"
    ...
    def load_request(self):
        ...
        self.le_data.setText(saved_entry.get("data"))
        self.le_json.setText(saved_entry.get("json"))
